...
public final class ClassUtils {
    ...
    public static <T> List<T> getObjectsWithInterface(Context context, Class<T> clazz, String path) {
        ...
        try {
            ...
            if (objectList.size() == 0) {
                LogUtils.eTag(TAG, "No files were found, check your configuration please!");
            }
        } catch (Exception e) {
            e.getStackTrace();
            LogUtils.eTag(TAG, "getObjectsWithInterface error, " + e.getMessage());
        }
    ...
    public static List<String> getFileNameByPackageName(Context context, String packageName) throws PackageManager.NameNotFoundException, IOException {
        ...
        for (String path : getSourcePaths(context)) {
            ...
            try {
                ...
                LogUtils.eTag(TAG, "Scan map file in dex files made error.", ignore);
            } finally {
                if (null != dexfile) {
                    try {
                        dexfile.close();
        ...
        LogUtils.eTag(TAG, "Filter " + classNames.size() + " classes by packageName <" + packageName + ">");
        return classNames;
    }
    ...
    private static List<String> tryLoadInstantRunDexFile(ApplicationInfo applicationInfo) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && null != applicationInfo.splitSourceDirs) {
            ...
            LogUtils.dTag(TAG, "Found InstantRun support");
        } else {
            try {
                ...
                LogUtils.dTag(TAG, "InstantRun support error, " + e.getMessage());
            }
        }
    ...
    private static boolean isVMMultidexCapable() {
        ...
        LogUtils.iTag("galaxy", "VM with name " + vmName + (isMultidexCapable ? " has multidex support" : " does not have multidex support"));
