 ...
 **/
public class FgtIOSVM extends BaseViewModel<List<DataEntity>, GankIODM> {

    private int mPage = 1;
    private static final String CATEGORY = "iOS";

    public FgtIOSVM(@NonNull Application application) {
        super(application);
    ...
    public final ItemBinding<ItemFgtIOSVM> singleItem = ItemBinding.of(com.karashok.mvvm.module_gankio.BR.itemFgtIOSVM, R.layout.item_gank_ios);

    public final ObservableList<ItemFgtIOSVM> items = new ObservableArrayList<>();
    ...
    public void dataSuccess(final List<DataEntity> data) {
        ...
        if (data != null && !data.isEmpty()) {
            Observable.fromIterable(data)
                    .map(new Function<DataEntity, ItemFgtIOSVM>() {
                        @Override
                        public ItemFgtIOSVM apply(DataEntity dataEntity) throws Exception {
                            return new ItemFgtIOSVM(dataEntity);
                        }
                    })
                    .toList()
                    .subscribe(new Consumer<List<ItemFgtIOSVM>>() {
                        @Override
                        public void accept(List<ItemFgtIOSVM> itemFgtIOSVMS) throws Exception {
                            items.addAll(itemFgtIOSVMS);
                        }
                    });
        }
        mPage++;
